...
class Api:
    ...
    def get_bin_id(self, market, num=None, name=None):
        response = requests.get("https://www.predictit.org/api/Market/" + str(market) + "/Contracts",
                                headers={'Authorization': 'Bearer ' + self.token}).json()
        print(num)
        if num is not None:
            return response[num]['contractId'], response[num]['contractName']
        elif name:
            for contract in response:
                if name.lower().strip() in contract['contractName'].lower():
                    return contract['contractId'], contract['contractName']
        return None, None

    def make_trade(self, contract, num, price, long):

        print(contract, num, price, long)
        print("quantity=" + str(num) + "&pricePerShare=" + str(price) + "&contractId=" + str(contract) + "&tradeType=" + str(int(long)))
        response = requests.post("https://www.predictit.org/api/Trade/SubmitTrade", data="quantity=" + str(num) + "&pricePerShare=" + str(price) + "&contractId=" + str(contract) + "&tradeType=" + str(int(long)),
    ...
    def make_trade_bin(self, market, bin, num, price, long):
        if type(bin) == str:
            contract, name = self.get_bin_id(market, name=bin)
        elif type(bin) == int:
            contract, name = self.get_bin_id(market, num=bin)
        else:
            return None
        print(contract)
        if not contract:
            return None
        print("making trade")
        return self.make_trade(contract, num, price, long), name

    def purchase_spread(self, market, spread, prices):
        start = self.get_money()
        if len(spread) != len(prices):
            return False
        traded = []
        for i, num in enumerate(spread):
            if num > 0:
                made, name = self.make_trade_bin(market, i, num, int(prices[i] * 100), False)
                traded.append(made)
        time.sleep(1)
        end = self.get_money()
        print(end, start)
        return traded, round(float(end[1:]) - float(start[1:]), 2)

    def get_money(self):
        return requests.get("https://www.predictit.org/api/User/Wallet/Balance", headers={'Authorization': 'Bearer ' + self.token,
                                          'Content-Type': 'application/x-www-form-urlencoded'}).json()['accountBalance']